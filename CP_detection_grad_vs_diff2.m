...
for j = 1:9
    ...
    for k = 1:length(t11)
        ...
        filt = ((t21 >=t11(k) & t21 <= t12(k)) | (t22 >= t11(k) & t22 <= t12(k)) |...
                (t21 <= t11(k) & t22 >= t12(k))   ) & ID2 == ID1(k);
    ...
    %         sub = sub+1; subplot(nrow,ncol,sub); hold on;
            ...
            legend(h(:),{'Ta','grad-10','diff-mean2'},'Location','best')
            ...
            if strcmp(choice,'y')
                version = version + 1;
                export_fig([figout,'missing_explanation_v',num2str(version),'.png'],'-dpng','-r200')
            end
            clf; hold on;
...
diff2_far = zeros(size(t21,9));
version = 0;
plt = 0;
for j = 1:9
    j
    t21 = CP_tstart{3,j}; % diff-mean2
    t22 = CP_tend{3,j};
    ID2 = CP_sID{3,j};
    ...
    %         sub = sub+1; subplot(nrow,ncol,sub); hold on;
            ...
            legend(h(:),{'Ta','grad-10','diff-mean10'},'Location','best')
...
diff2_mis_meaning = {'1 = CPs in grad is also in diff; 2 = not in diff due to smoothing; 3 = not in diff due to resolution'};
diff2_far_meaning = {'1 = CPs in diff is also in grad; 0 = not in grad due to slope value'};
save([matdir,'Saildrone_cold_pools_2018_2023_3Tcrits.mat'],'diff2_mis','diff2_far',...
    'diff2_mis_meaning','diff2_far_meaning','-append')
