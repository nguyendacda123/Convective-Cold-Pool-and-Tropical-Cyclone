...
figout = [figdir,'Cold_pool_R1/Maps_in_TCs/SHIPS_VWS/']; 
...
for opt = 3
    ...
    load([matdir,'Active_gauges_and_CPs_in_SHIPS_VWS.mat'],'lon_center','lat_center',...
        ...
        'mdist_500','mdist_200','Dist_min','tDist_min','sst_gauge','d2land_center')
    ...
    if opt == 2 
        ...
        tag = '_VWS_offshore_gauge';
    elseif opt == 3
        tag = '_VWS_offshore_TCs'
    else 
        tag = '_VWS';
    end
    ...
    filt0 = loncp >= LONR1 & loncp <= LONR2 & latcp >= LATR1 & latcp <= LATR2 & cp_flag == 1;
    if opt == 3
        filt0 = filt0 & d2land >=250;
    end
    R = R(filt0);
    phi = phi(filt0);
    type = type(filt0);
    ...
    export_fig([figout,'CP_locations_in',tag,'.png'],'-dpng','-r200')
end
...
for opt = 2:3
    ...
    set(gcf,'Color','w'); pause
    export_fig([figout,'CP_quadrants_barplot',tag,'.png'],'-dpng','-r200')    
    pause(0.1);
    clf;
end
...
for opt = 1:3
    ...
    for opt = 1:3
        ...
        set(gca,'FontSize',14,'Xtick',1:4,'XtickLabel',{'DR','UR','UL','DL'});
