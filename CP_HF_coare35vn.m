%modified line 304 to saturate roughness length

%function A=coare35vn(u,zu,t,zt,rh,zq,P,ts,Rs,Rl,lat,zi,rain,cp,sigH)
% function A=coldpool3_HF_coare35vn(pth,fname)
function A=CP_HF_coare35vn(x2)

%INPUT:
%loca = name of input file (see coare3a_infile_clean.m)
%pth = path to COARE input file (see coare3a_infile_clean.m)
%OUTPUT:
%sensible heat flux (W/m^2, positive means heat lost by ocean) is third variable: A(:,3);
%latent heat flux (W/m^2, positive means heat lost by ocean) is fourth variable: A(:,4);
...
zu=zu(:);zt=zt(:);zq=zq(:);cp=cp(:);sigH=sigH(:);
rain=rain(:);
N=length(u);
...
Ad=0.15;  %Sea-state/wave-age dependent coefficients
Bd=2.2;
...
A=[usr tau hsb hlb hbb hsbb hlwebb tsr qsr zot zoq Cd Ch Ce  L zet dter dqer tkt Urf Trf Qrf RHrf UrfN Rnl Le rhoa UN U10 U10N Cdn_10 Chn_10 Cen_10 RF Qs Evap T10 Q10 RH10 zo];
