...
figout = [figdir,'Cold_pool_R1/Maps_in_TCs/IBTrACS/']; 
...
% load data              
    ...
    export_fig([figout,'CP_locations_in_TCs',tag,'.png'],'-dpng','-r200')
end
...
% load data              
    ...
    for opt = 1:3
        ...
        for d = 1:length(datname)
            ...
            for i = 1%:length(Rplot)
                ...
                set(gca,'Xtick',1:4,'XtickLabel',{'FR','RR','RL','FL'},'FontSize',12,'Ygrid','on')
    ...
    set(gcf,'Color','w'); pause
    export_fig([figout,'CP_in_TC_quadrants_barplot',tag,'.png'],'-dpng','-r200')    
    pause(0.1);
    clf;
end
...
for d = 4%:4
    ...
    Rmax = reshape(repmat(Rmax_center,[1, nb]),[],1);

    cl1 = {my_color('dull light blue'),my_color('deep sky blue'),...
        my_color('dark steel blue'),my_color('medium slate gray')};
    cl2 = {my_color('hot pink'),my_color('magenta'),...
        my_color('violet'),my_color('red')};
    bins = 0.5:1:4.5; % for quadrant count
    ...
    for opt = 1:4
        ...
        elseif opt == 2
            filt1 = Vtrans >= prctile(Vtrans,50);
            filt2 = Vtrans < prctile(Vtrans,50);
            cats = {'fast','slow'};
            tit = 'CPs vs. TC translation speed';
        elseif opt == 3
            filt1 = TCI <=0 ;
            filt2 = TCI > 0;   
            cats = {'weakening','strengthening'};
            tit = 'CPs vs. TC intensity change';
        elseif opt == 4
            filt1 = Rmax <= prctile(Rmax,50);
            filt2 = Rmax > prctile(Rmax,50);
            cats = {'small','large'};
            tit = 'CPs vs. TC size';
        end
    ...
    xlabel('TC quadrant');
    set(gcf,'Color','w'); pause
    export_fig([figout,'TC_quadrant_coldpool_distribution_vs_Vmax_Vtrans_TCI',tag,'.png'],'-dpng','-r200')

end

%%
              
load([matdir,'Active_gauges_and_CPs_in_TC_coordinates.mat'],'d2land_center','Rmax_center')
figure; 
hist(Rmax_center(d2land_center > 0));
title('Distribution of Radius of Maximum Wind');
xlabel('km'); ylabel('count');
...
datname = {'0.5% thresholds','1% thresholds','2% thresholds'};
% TC_days = sum(filt0)*600/86400;
...
Msize = 1000;
sub = 0;
cl = {my_color('pink'),my_color('magenta'),my_color('violet')};
tab = [];

tags = {'_0.5prctile','','_2prctile'};
for opt = 1:3



    % load data              
    load([matdir,'Active_gauges_and_CPs_in_TC_coordinates',tags{opt},'.mat'],'lon_center','lat_center',...
        'time_center','Vmax_center','Vtrans_center','TCI_center','lon_gauge','lat_gauge','dist_gauge',...
    ...
    title(datname{opt},'FontWeight','normal','FontSize',16)
...
set(gcf,'Color','w'); pause
export_fig([figout,'Sensitivity_to_thresholds.png'],'-dpng','-r200')    
pause(0.1);
